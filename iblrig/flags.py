...
def create_transfer_flags(root_data_folder, force=False, file_list=None):
    create_other_flags(
        root_data_folder, "transfer_me.flag", force=False, file_list=None
    )
...
def create_compress_video_flags(
    root_data_folder, flag_name="compress_video.flag", clobber=False
):
    ...
    for video_path in video_paths:
        ...
        for vfile in vfiles:
            logger_.info(
                str(vfile.relative_to(ses_path)) + " added to " + str(flag_file)
            )
            write_flag_file(
                flag_file, file_list=str(vfile.relative_to(ses_path)), clobber=clobber
            )
...
def create_audio_flags(root_data_folder, flag_name):
    ...
    for audio_path in audio_paths:
        ...
        for afile in afiles:
            logger_.info(
                str(afile.relative_to(ses_path)) + " added to " + str(flag_file)
            )
